...
const OK_ID: usize = 101;
const CANCEL_ID: usize = 102;
...
pub fn open(self: *Self, window: *Window, surface: *CoreSurface, mode: Mode, initial: [:0]const u8) !void {
    ...
    self.edit_hwnd = CreateWindowExW(0, edit_class, initial_w.ptr, WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL | WS_TABSTOP, 10, 10, width - 20, 28, self.hwnd, @ptrFromInt(EDIT_ID), hinstance, null) orelse return error.Win32Error;

    const button_class = std.unicode.utf8ToUtf16LeStringLiteral("BUTTON");
    _ = CreateWindowExW(0, button_class, std.unicode.utf8ToUtf16LeStringLiteral("OK"), WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_DEFPUSHBUTTON, width - 170, 50, 70, 28, self.hwnd, @ptrFromInt(OK_ID), hinstance, null) orelse return error.Win32Error;
    _ = CreateWindowExW(0, button_class, std.unicode.utf8ToUtf16LeStringLiteral("Cancel"), WS_CHILD | WS_VISIBLE | WS_TABSTOP, width - 90, 50, 70, 28, self.hwnd, @ptrFromInt(CANCEL_ID), hinstance, null) orelse return error.Win32Error;
